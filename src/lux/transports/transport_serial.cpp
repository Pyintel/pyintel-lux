...
static uint8_t s_ser_rx_buf[LUX_MAX_PAYLOAD_SIZE + LUX_MESH_HEADER_SIZE + LUX_HEADER_SIZE];
static size_t  s_ser_rx_index = 0;
static size_t  s_ser_expected_len = 0;
static bool    s_ser_in_sync = false;
...
static int serial_receive(uint8_t *buf, size_t max_len, uint16_t *src_node) {
    ...
    while (s_serial_stream->available() > 0) {
        ...
        if (!s_ser_in_sync) {
            ...
            } else if (s_ser_rx_index == 1) {
                if ((s_ser_rx_buf[0] == LUX_MESH_SYNC_0 && b == LUX_MESH_SYNC_1) ||
                    (s_ser_rx_buf[0] == LUX_SYNC_0 && b == LUX_SYNC_1)) {
                    s_ser_rx_buf[1] = b;
                    s_ser_rx_index = 2;
                    s_ser_in_sync = true;
                    s_ser_expected_len = 0;
        ...
        if (s_ser_rx_index < sizeof(s_ser_rx_buf)) {
            s_ser_rx_buf[s_ser_rx_index++] = b;
        } else {
        ...
        if (s_ser_expected_len == 0) {
            if (s_ser_rx_buf[0] == LUX_MESH_SYNC_0) {
                // Mesh frame: Mesh header (12) + Inner Lux Header (14)
                if (s_ser_rx_index >= (LUX_MESH_HEADER_SIZE + LUX_HEADER_SIZE)) {
                    uint8_t inner_payload_len = s_ser_rx_buf[LUX_MESH_HEADER_SIZE + 11];
                    s_ser_expected_len = LUX_MESH_HEADER_SIZE + LUX_HEADER_SIZE + inner_payload_len;
                }
            } else {
                // Raw Lux frame: 14 bytes header + payload
                if (s_ser_rx_index >= LUX_HEADER_SIZE) {
                    uint8_t payload_len = s_ser_rx_buf[11];
                    s_ser_expected_len = LUX_HEADER_SIZE + payload_len;
                }
            }
        }

        // Full packet assembled
        if (s_ser_expected_len > 0 && s_ser_rx_index >= s_ser_expected_len) {
            size_t total = s_ser_expected_len;
            if (total > max_len) total = max_len;
            memcpy(buf, s_ser_rx_buf, total);

            if (src_node && s_ser_rx_buf[0] == LUX_MESH_SYNC_0) {
                lux_mesh_envelope_t *env = (lux_mesh_envelope_t *)s_ser_rx_buf;
