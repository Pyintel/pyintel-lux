        ...
        with serial.Serial(port, baud, timeout=1) as ser:
            ...
            while True:
                ...
                while True:
                    idx = buf.find(SYNC)
                    if idx == -1:
                        # Keep last 1 byte in case SYNC spans read boundaries
                        if len(buf) > 1:
                            buf = buf[-1:]
                        break
                    ...
                    frame_bytes = buf[:frame_len]
                    frame_count += 1
                    console.print(f"[bold cyan][UART RX][/bold cyan] Ingested Frame #{frame_count} ({frame_len} bytes)")
