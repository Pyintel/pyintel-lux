...
#define LUX_PROGMEM PROGMEM
#define LUX_READ_WORD_PGM(addr) pgm_read_word(addr)
#else
#define LUX_PROGMEM
#define LUX_READ_WORD_PGM(addr) (*(addr))
#endif

/* ── 256-entry CRC-16/CCITT-FALSE Lookup Table (LUT) ─────────────────── */
static const uint16_t crc16_table[256] LUX_PROGMEM = {
...
uint16_t lux_crc16(const uint8_t *data, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        uint16_t table_val = LUX_READ_WORD_PGM(&crc16_table[index]);
        crc = (crc << 8) ^ table_val;
    }
    return crc;
}
